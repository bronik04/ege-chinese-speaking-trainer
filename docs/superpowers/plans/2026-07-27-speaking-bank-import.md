...
### Задача 10a: снять замороженные счётчики каталога

Обнаружено при выполнении: три проверки фиксировали размер каталога числом и падали от любого пополнения.
Задача не была предусмотрена планом.

**Файлы:**
- Изменить: `tests/unit/test_variants.py:15`, `tests/unit/test_variants.py:50`
- Изменить: `tests/integration/test_materials.py:95`

- [x] `test_all_variants_follow_current_speaking_format` требовал ровно 7 записей в `index.json`.
  Проверка разделена: новый `test_index_covers_every_variant_file` сверяет, что записи индекса совпадают
  с файлами вариантов на диске, а формат проверяется для всех записей без счётчика.
- [x] `test_variant_images_are_optimized` требовал ровно 42 webp и суммарный вес меньше 3 МБ. Теперь —
  шесть изображений на вариант, каждое меньше 256 КБ и средний вес меньше 72 КБ: бюджет на снимок
  не приходится двигать при каждом пополнении.
- [x] `test_user_creates_uploads_and_publishes_standalone_task` требовал ровно 7 материалов в выдаче
  подтверждённому пользователю. Теперь сверяется множество идентификаторов с `content/variants/index.json`.
