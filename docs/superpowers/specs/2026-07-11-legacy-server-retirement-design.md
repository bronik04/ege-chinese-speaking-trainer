...
- `tests/integration/test_server.py` переезжает в `legacy/tests/test_server.py` и импортирует `legacy.server`.
- `legacy/README.md` описывает только ручной запуск и ручную проверку legacy-слоя. Эти команды не входят в
